...
test_psgi 
	...
	client => sub {
		my $cb = shift;
		my $req = HTTP::Request->new(POST => '/redis/set/foo/1?callback=Callback');
		my $res = $cb->($req);
		is $res->code, 200;
...
test_psgi 
	...
	client => sub {
		my $cb = shift;
		my $req = HTTP::Request->new(GET => '/redis/del/foo?callback=Callback');
		my $res = $cb->($req);
		is $res->code, 200;
...
test_psgi 
	...
	client => sub {
		my $cb = shift;
		my $req = HTTP::Request->new(POST => '/redis/get/foo?callback=Callback');
		my $res = $cb->($req);
		is $res->code, 200;
